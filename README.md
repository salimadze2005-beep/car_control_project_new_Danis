...
`Autopilot target (m/s)` и нажмите `Apply speed`. Подтверждение появляется только
после ответа controller. Диапазон панели 0.1–5.0 m/s; это диапазон ввода,
...
```bash
python3 tools/fsds_speed.py 2.0
```
