...
def prepare(destination):
    ...
    check = subprocess.run(['git', 'apply', '--reverse', '--check', str(patch)], cwd=destination, capture_output=True)
    if check.returncode:
        git('apply', '--check', patch, cwd=destination)
        git('apply', patch, cwd=destination)
